...
*/
export function setupRaster (canvas, sg, camera, gn2, animationNodes) {
  const gl = canvas.getContext('webgl')
  const setupVisitor = new RasterSetupVisitor(gl)
  setupVisitor.setup(sg)
  ...
  visitor.textureshader = textureShader
  loop(sg, gl, animationNodes, camera, phongShader, textureShader, visitor)
}

function loop (sg, gl, animationNodes, camera, phongShader, textureShader, visitor) {
  function simulate (deltaT) {
    for (let animationNode of animationNodes) {
      animationNode.simulate(deltaT)
    }
...
export function teardownRaster () {
